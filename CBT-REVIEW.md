...
- OFF hides the student's scores, answer ledger, security summary and report download. Completion, save status, retry and feedback remain available. The result still saves automatically. Open student report pages react to changes in the setting. Policy-read failure hides the report; downloading checks the setting again.
...
- Browser preview was blocked in this environment and a local Chromium download timed out. No visual mobile/desktop QA, actual PDF download, physical phone screen-timeout test or live Firestore test is claimed.

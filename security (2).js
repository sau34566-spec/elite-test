...
(function () {
  ...
  function report(reason, severity = "hard", details = {}) {
    if (!isActive()) return;
    // visibility, blur, fullscreen and resize can all fire for one action.
    // Keep the whole away/overlay episode as exactly one violation.
  ...
  function viewportIsNormal() {
    if (!baselineWidth || !baselineHeight) return true;
    const size = viewportSize();
    return (
      size.width / baselineWidth >= config.widthRatio &&
      size.height / baselineHeight >= config.heightRatio
    );
  }
